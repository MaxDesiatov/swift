...
Used by `WASISysroot` (`wasisysroot.py`) and
`EmscriptenLLVMRuntimeLibs` (`emscriptensysroot.py`).
...
Note: callers build `compiler_rt_source_dir` from
`os.path.dirname(self.source_dir)` plus a product-specific suffix
(`llvm-project/compiler-rt` for WASI, `compiler-rt` for Emscripten —
coincidentally the same path today). Renaming either product's
...
def build_runtimes(args, toolchain, runtimes_source_dir, runtimes_build_dir,
    ...
    `append_platform_cmake_options` sets the platform-specific `CMAKE_*`
    options (sysroot, system name, compilers, triples, flags); this helper
    owns the shared `LIBCXX_*` / `LIBCXXABI_*` knobs and the build +
    install steps.
    """
...
def build_compiler_rt(args, toolchain, compiler_rt_source_dir,
    ...
    Same platform-callable contract as `build_runtimes`. Keeps the
