...
# On-disk `.artifactbundle` directory name (without the
# `.artifactbundle` suffix) shared by every wasm Swift SDK product (WASI +
...
CANONICAL_BUNDLE_NAME = "swift-wasm-sdk"
...
def generate_swift_sdk(swift_run, source_dir, build_dir, triple, sysroot,
    ...
    Each call appends one Swift SDK artifact to the `.artifactbundle`
    named `bundle_name`. `--incremental` is always passed so subsequent
    invocations (for other wasm triples) merge into the existing bundle's
    `info.json` instead of overwriting it; pass
    `CANONICAL_BUNDLE_NAME` to land in the shared bundle.

    `sdk_name` is the artifact ID inside `info.json`. When omitted,
    swift-sdk-generator computes its own per-triple `defaultArtifactID`
    (e.g. `<swift_version>_wasm`, `..._wasm-threads`,
    `..._wasm-emscripten`), which already keeps artifact keys distinct.
    """
    run_args = [
        swift_run,
    ...
    # Pass only the override: `shell.call` merges into `os.environ` itself
    # and echoes the given env dict to stderr, so `dict(os.environ)` here
    # would dump every host var (CI secrets included) into the build log.
