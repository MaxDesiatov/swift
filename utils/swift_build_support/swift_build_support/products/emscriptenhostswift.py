...
class EmscriptenHostSwift(product.Product):
    """Cross-compiles `swift-frontend` so the compiler binary itself runs
    *hosted on* a WebAssembly runtime (wasm32-unknown-emscripten), driven by
    Node. This is the Swift half of the wasm-hosted toolchain; the LLVM/clang
    half is EmscriptenHostLLVM. Distinct from EmscriptenStdlib /
    EmscriptenSwiftSDK, which build the Swift stdlib *for* the wasm target.

    Prerequisites (two-flag invocation): this product consumes already-built
    sibling build dirs and is gated on a *different* flag than the products
    that produce them, so they must already exist when it runs. Build them in
    this or a prior invocation:
      * --build-emscripten-host-llvm  -> emscriptenhostllvm-<host> (LLVM_DIR /
        Clang_DIR + the lld archives the shim imports)
      * --build-emscripten-stdlib     -> emscriptenstdlib-<host> (the wasm
        target stdlib / -resource-dir) and emscripten-sysroot/... (the -sdk
        sysroot)
    The native build tree (swift-<host>, llvm-<host>) supplies the
    CROSSCOMPILE bootstrapping swiftc and the native llvm/clang-tblgen.

    Drives an `emcmake cmake` configure + build using the in-tree
    swift/cmake/caches/EmscriptenHostSwift.cmake cache file and the
    EmscriptenHostSwiftLLDShim.cmake top-level include. The wasm cmark
    dependency is built as a private sub-step (the shared CMark product is
    native-only), leaving it untouched.
    """

    @classmethod
    def product_source_name(cls):
        # Cross-compile the same Swift source the native build uses.
        return "swift"
    ...
    def get_dependencies(cls):
        # EmscriptenHostLLVM supplies LLVM_DIR/Clang_DIR + the lld archives;
        # EmscriptenSwiftSDK transitively pulls native swift.Swift + the
        # emscripten stdlib/sysroot. Listing both makes the build-graph
        # topological sort order this product after them. (EmscriptenSwiftSDK
        # is gated on --build-emscripten-stdlib, a different flag, so under
        # --build-emscripten-host-swift alone it is scheduled-but-not-built;
        # the fail-loud validation in build() guards the missing-deps case.)
        return [emscriptenhostllvm.EmscriptenHostLLVM,
                emscriptenswiftsdk.EmscriptenSwiftSDK]

    def should_build(self, host_target):
        # Build the wasm-host frontend once on the native build host: emcc is
        # the cross compiler and the reused native swiftc/llvm-tblgen must run
        # on the build machine, so cross-compile-host iterations are skipped.
    ...
    def build(self, host_target):
        ...
        # Cross-LLVM (EmscriptenHostLLVM) build dir: LLVM_DIR / Clang_DIR live
        # under lib/cmake, and the lld archives the shim imports under lib.
        ...
        # Prebuilt wasm target stdlib (EmscriptenStdlib build dir): used as
        # SWIFT_PATH_TO_SWIFT_SDK, and its lib/swift is the -resource-dir.
        ...
        # The built emscripten -sdk sysroot (EmscriptenSysroot install path).
        # FIX-3's <xlocale.h> compat headers live under include/compat there.
        built_sysroot = \
        ...
        # Native build-tree tools (CROSSCOMPILE bootstrapping swiftc + the
        # llvm/clang-tblgen that must run on the build machine), honoring the
        # --native-*-tools-path overrides.
        ...
        # Build the wasm cmark dependency first (its config is read during the
        # frontend configure below).
        ...
        # NOTE: EmscriptenHostSwift.cmake is a CMake `-C` initial cache whose
        # `set(... CACHE ...)` is no-FORCE, so editing it does NOT take effect
        # on an existing build dir even with --reconfigure; remove the build
        # dir (fresh configure) after changing the cache file.
    ...
    def _build_cmark_wasm(self, host_target, emcmake, cmark_source_dir,
                          build_root):
        """Build cmark for the wasm host as a private sub-step and return its
        build dir (for SWIFT_PATH_TO_CMARK_BUILD). The shared native CMark
        product is native-only, so we build a fresh wasm copy here rather than
        editing it."""
    ...
    def _needs_configure(self, build_dir):
        # Existence-based reconfigure guard (mirrors EmscriptenHostLLVM /
        # cmake_product.py): reconfigure when asked, or when either the cache
        # or the generator output is missing.
    ...
    def _emcmake_path(self):
        ...
        # EmscriptenHostLLVM / EmscriptenSysroot); only fall back to PATH when
        # it is unset.
    ...
    def _emcc_cache_sysroot(self, emcmake):
        # SWIFT_EMSCRIPTEN_SYSROOT_PATH is the emcc *cache* sysroot
        # (<emcc cache>/sysroot, for host-tool C++ headers). Ask Emscripten for
        # its own cache dir via `em-config CACHE` (em-config sits next to
        # emcmake) rather than doing path math: under Homebrew, `emcmake` is a
        # bash wrapper whose realpath stays in bin/, while the real cache lives
        # under libexec/, so a dirname-based derivation points at a
        # nonexistent bin/cache. em-config resolves correctly through the
        # wrapper.
        em_dir = self.args.emscripten_path or \
            os.path.dirname(os.path.realpath(emcmake))
        if self.args.dry_run:
            # Don't invoke tools under --dry-run; the emitted command is for
            # inspection only. With an explicit --emscripten-path this matches
            # what `em-config CACHE` would return (its cache is <path>/cache).
    ...
    def _cache_file_path(self):
        ...
        # build). self.source_dir is <source_root>/swift, so it is a direct
        # child here (unlike EmscriptenHostLLVM, whose source is in
        # llvm-project).
