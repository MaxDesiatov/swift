...
#else
  // No host threads (e.g. the single-threaded WebAssembly host build):
  // constructing a started std::thread aborts at runtime on a no-pthread
  // Emscripten host. embedBitcode reads only the llvm::Module and IRGenOptions,
  // never the SILModule, so freeing the SILModule first cannot affect it — the
  // two are independent in either order (which is why the original code is free
