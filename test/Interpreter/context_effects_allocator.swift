...
struct BumpAllocator: Allocation {
  ...
  init(region: UnsafeMutableRawBufferPointer) { self.region = region }
...
func testSystemAllocator() {
  ...
  do {
    let buf = allocateAndWrite(42, byteCount: 16)
...
func testBumpAllocator() {
  ...
  do {
    let buf = allocateAndWrite(99, byteCount: 32)!
...
func testNestedHandlers() {
  ...
  do {
    ...
    let sysBuf = allocateAndWrite(11, byteCount: 8)!
    ...
    print("outer (system) in arena: \(sysInArena), byte: \(sysBuf[0])")
    deallocateBuffer(sysBuf)

    do {
      // This allocation uses BumpAllocator (inner overrides).
      let bumpBuf = allocateAndWrite(22, byteCount: 8)!
      ...
      print("inner (bump) in arena: \(bumpInArena), byte: \(bumpBuf[0])")
...
func testExistentialPath() {
  ...
  defer { arena.deallocate() }
  do {
    let buf = allocateExistential(byteCount: 256)!
    buf[0] = 77
...
func testBumpOOM() {
  ...
  do {
    ...
    let third = perform { (a: inout some Allocation) in
