...
// The variable row lowers into the substitution as a fresh generic parameter.
// CHECK-LABEL: sil hidden [ossa] @$s{{.*}}7forward{{.*}} : $@convention(thin) <E where E : Effect> (@guaranteed @noescape @callee_guaranteed @substituted <τ_0_0> () -> () for <E>) -> ()
